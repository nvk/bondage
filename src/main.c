...
bondage_usage(FILE *stream, const char *progname)
{
  fprintf(stream,
          ...
          "  %s [--config <path>] chain <profile> [config] [-- args...]\n"
          "  %s [--config <path>] exec <profile> [config] [-- args...]\n"
          "  %s [--config <path>] argv <profile> [config] [-- args...]  (alias for chain)\n"
...
bondage_doctor_usage(const char *progname)
{
  fprintf(stderr,
          ...
          "  %s --config \"$BONDAGE_CONF\" doctor\n"
          "  %s --config \"$BONDAGE_CONF\" repin-globals   # if doctor suggests it\n"
          "  %s --config \"$BONDAGE_CONF\" repin <profile> # if doctor suggests it\n"
          "  %s --config \"$BONDAGE_CONF\" verify <profile>\n"
          "  %s --config \"$BONDAGE_CONF\" chain <profile> -- --help\n\n"
          "Notes:\n"
...
bondage_main(int argc, char **argv)
{
  ...
  case BONDAGE_CMD_DOCTOR:
    if (command_argc >= 1 &&
        (strcmp(command_argv[0], "--help") == 0 || strcmp(command_argv[0], "-h") == 0)) {
